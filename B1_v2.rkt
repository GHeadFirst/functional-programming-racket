...
;(-4) ; gives out -4 as a number (false) because it is a procedure call of a function called-4

(- 4) ; subtracts 4 from nothing, gives out positive 4 (false) gave out -4

-4 ;gives out -4 since it is already as simple as possible (cannot be
; broken down further

- 4 ;results in an error as it is a function call without () parenthesis (false) gives out #<procedure:-> and then 4 as a positive number
...
(sign -2) ; expected result -1
(sign -1) ; expected result -1 
(sign 0) ; expected result 0
(sign 1) ;expected result 1
(sign 2) ;exoected result  1
(sign 5) ; expected result 1
(sign -3) ; expected result -1
...
#|
(define (sumSquareBigger num1 num2 num3)
  (define a2 (square num1))
  (define b2 (square num2))
  (define c2 (square num3))
  (max (+ a2 b2 ) (+ b2 c2) (+ a2 c2) ))
...
(sumSquareBigger 0 -5 -10) ; expected result 125 since 0 and -5 are the two "larger" numbers
...
(displayln "exercise A5")
(displayln "")

;A5
(define (mynot x)
  (cond
    [(equal? x #f) #t] ;checks if x is equal to #f a "falsey value" which is always false (unless #f is given) therefore the conditon never evaulates to true and true is not given back
    [else #f]))
...
(mynot 5) ;#f
(mynot #t) ;#f
(displayln "normal not test cases for comparison")
(not #f)
(not 5)
(not #t)
...
(if (and (= 1 1) (= 2 2))
    "This will evaluate since both conditions are true"
    "false case")
(if (and (> 1 2) (= 1 1))
    "With this and, both conditions have to be evaulated 1 == 1 && 1 > 2 and both need to evaluate to true for this to print and since 1 > 2 is false the second condition won't be evaluated or looked at and will return false"
    "false case")
(if (or (= 1 1) (= 2 3))
     "this will short circuit and 2 == 3 will not even be evaluated since with or only one condition needs to  be true"
     "false statement")
(if (or (> 1 2) (> 2 3))
    "since both evaluate to false, this will not print as both conditions are evaluated since the first one evaluates to false"
    "false")
